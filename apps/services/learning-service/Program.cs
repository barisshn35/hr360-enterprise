...
builder.Services.AddAuthorization(options =>
{
    ...
    // ayni ize karsilik geliyor (learning:manage).
    options.AddPolicy("RequireHrAdmin", policy =>
        policy.RequireAssertion(ctx =>
            ctx.User.IsInRole("hr-admin") || ctx.User.IsInRole("tenant-admin") ||
            ctx.User.IsInRole("platform-admin") || ctx.User.IsInRole("ext-learning-manage")));
    options.AddPolicy("RequireManagerOrAbove", policy =>
        policy.RequireAssertion(ctx =>
            ...
            ctx.User.IsInRole("ext-learning-manage")));
});
...
app.MapGet("/health", () => Results.Ok(new { status = "ok", service = "learning-service" }));
app.MapControllers();
app.MapMetrics();

app.Run();
// gitlab 520 retry dogrulama
